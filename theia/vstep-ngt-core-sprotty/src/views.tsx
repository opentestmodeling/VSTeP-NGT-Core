...
import { Point, PolylineEdgeView, RenderingContext, SEdge, SShapeElement, toDegrees, IView, SPort } from "sprotty";
import { injectable } from 'inversify';

@injectable()
export class PolylineDependencyEdgeView extends PolylineEdgeView {
...
@injectable()
export class PolylineInheritanceEdgeView extends PolylineEdgeView {

    protected renderAdditionals(edge: SEdge, segments: Point[], context: RenderingContext): VNode[] {
        const p1 = segments[segments.length - 2]
        const p2 = segments[segments.length - 1]
        return [
            <path class-inheritance-edge-arrow={true} d="M 18,-9 L 0,0 L 18,9 Z"
                  transform={`rotate(${this.angle(p2, p1)} ${p2.x} ${p2.y}) translate(${p2.x} ${p2.y})`}/>
        ]
    }
...
@injectable()
export class VstepNgtTestTargetView implements IView {
    render(node: Readonly<SShapeElement>, context: RenderingContext): VNode {
        const dx = Math.max(node.size.width, 0) / 2;
        const dy = Math.max(node.size.height, 0) / 2;
        return <g>
            <ellipse class-vstepngt-target={true} cx={dx} cy={dy} rx={dx} ry={dy}></ellipse>
